...
class PGSNet(nn.Module):
    def __init__(self, in_dim=3, new_model=False):
    ...
    # @autocast()
    def forward(self, image: torch.Tensor, aolps: torch.Tensor, dolps: torch.Tensor, non_similarity_map: list = None):
        ...
        if self.new_model:
            image_cr4 = image_cr4 + image_cr4 * F.interpolate(non_similarity_map[0], size=image_cr4.size()[2:], mode='bilinear', align_corners=True)
            image_cr3 = image_cr3 + image_cr3 * F.interpolate(non_similarity_map[0], size=image_cr3.size()[2:], mode='bilinear', align_corners=True)
            image_cr2 = image_cr2 + image_cr2 * F.interpolate(non_similarity_map[0], size=image_cr2.size()[2:], mode='bilinear', align_corners=True)
            image_cr1 = image_cr1 + image_cr1 * F.interpolate(non_similarity_map[0], size=image_cr1.size()[2:], mode='bilinear', align_corners=True)
            # image_t4 = image_t4 * F.interpolate(non_similarity_map[0], size=image_t4.size()[2:], mode='bilinear', align_corners=True)
        else:
            pass
        ...
        if self.new_model:
            aolp_cr4 = aolp_cr4 * F.interpolate(non_similarity_map[1], size=aolp_cr4.size()[2:], mode='bilinear', align_corners=True)
            # aolp_t4 = aolp_t4 * F.interpolate(non_similarity_map[1], size=aolp_t4.size()[2:], mode='bilinear', align_corners=True)
        ...
        if self.new_model:
            dolp_cr4 = dolp_cr4 * F.interpolate(non_similarity_map[2], size=dolp_cr4.size()[2:], mode='bilinear', align_corners=True)
            # dolp_t4 = dolp_t4 * F.interpolate(non_similarity_map[2], size=dolp_t4.size()[2:], mode='bilinear', align_corners=True)
        else:
            pass
        ...
        outputs = (predict_c1, predict_t1, predict_c2, predict_t2, predict_c3, predict_t3, 
