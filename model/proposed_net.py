...
from .layers import OpticalAttentionModule
...
class Fusion_T(nn.Module):
    ...
    def forward(self, image_t, aolp_t, dolp_t):
        ...
        concat = torch.cat([cross_image_aolp, cross_image_dolp, cross_aolp_image, cross_aolp_dolp,
                            cross_dolp_image, cross_dolp_aolp], 2) # B, N, dim*6
...
class T_Predict(nn.Module):
    def __init__(self, embed_dim=576, out_dim=1):
        super(T_Predict, self).__init__()
        self.segmentation_token_inference = segmentation_token_inference(embed_dim)
        self.out_dim = out_dim
        self.map = nn.Linear(embed_dim, self.out_dim)

    def forward(self, t):
        B, N, _ = t.shape

        t_features_for_predict = self.segmentation_token_inference(t)
        map = self.map(t_features_for_predict).transpose(1, 2).contiguous().reshape(B, self.out_dim, int(np.sqrt(N - 1)), int(np.sqrt(N - 1)))
...
class Network(nn.Module):
    def __init__(self, in_dim=3):
        super().__init__()
        print('Proposed Network')
        
        #---------RGB----------
        # backbone
        self.conformer1 = Conformer(in_chans=in_dim, patch_size=16, channel_ratio=6, embed_dim=576, depth=12,
        ...
        # backbone
        self.conformer2 = Conformer(in_chans=in_dim, patch_size=16, channel_ratio=6, embed_dim=576, depth=12,
                                    num_heads=9, mlp_ratio=4, qkv_bias=True)
        ...
        # backbone
        self.conformer3 = Conformer(in_chans=in_dim, patch_size=16, channel_ratio=6, embed_dim=576, depth=12,
                                    num_heads=9, mlp_ratio=4, qkv_bias=True)
        ...
        self.cross_attention_module_rgb = OpticalAttentionModule(in_channels=3840, out_channels=3840//4, num_classes=1)
        self.cross_attention_module_aolp = OpticalAttentionModule(in_channels=3840, out_channels=3840//4, num_classes=1)
        self.cross_attention_module_dolp = OpticalAttentionModule(in_channels=3840, out_channels=3840//4, num_classes=1)
    ...
    def forward(self, image: tuple, aolps: tuple, dolps: tuple) -> dict:
        ...
        # rescale
        predict_opflow_rgb = F.interpolate(pred_based_rgb_opflow, size=image[0].size()[2:], mode='bilinear', align_corners=True)
        predict_opflow_aolp = F.interpolate(pred_based_aolp_opflow, size=image[0].size()[2:], mode='bilinear', align_corners=True)
        predict_opflow_dolp = F.interpolate(pred_based_dolp_opflow, size=image[0].size()[2:], mode='bilinear', align_corners=True)
        predict_t = F.interpolate(predict_t, size=image[0].size()[2:], mode='bilinear', align_corners=True)
        predict4 = F.interpolate(predict4, size=image[0].size()[2:], mode='bilinear', align_corners=True)
        predict3 = F.interpolate(predict3, size=image[0].size()[2:], mode='bilinear', align_corners=True)
        predict2 = F.interpolate(predict2, size=image[0].size()[2:], mode='bilinear', align_corners=True)
        predict1 = F.interpolate(predict1, size=image[0].size()[2:], mode='bilinear', align_corners=True)

        outputs_dicts = {}
        outputs_dicts['conformer_cnnbased_rgb'] = predict_c1
        outputs_dicts['conformer_tbased_rgb'] = predict_t1
        outputs_dicts['conformer_cnnbased_aolp'] = predict_c2
        outputs_dicts['conformer_tbased_aolp'] = predict_t2
        outputs_dicts['conformer_cnnbased_dolp'] = predict_c3
        outputs_dicts['conformer_tbased_dolp'] = predict_t3
        outputs_dicts['gcc_out'] = predict_t
        outputs_dicts['AE4'] = predict4
        outputs_dicts['AE3'] = predict3
        outputs_dicts['AE2'] = predict2
        outputs_dicts['AE1'] = predict1 # final output
        outputs_dicts['opflow_rgb'] = predict_opflow_rgb
        outputs_dicts['opflow_aolp'] = predict_opflow_aolp
        outputs_dicts['opflow_dolp'] = predict_opflow_dolp
