...
class PreProcessing():
    ...
    def feature_pyramid_extract(self, frames: torch.Tensor) -> tuple:
        ...
        opflow_angle_mag = torch.cat([opflow_angle[:,None], opflow_magnitude[:,None]], dim=1)
        
        output = {
            'query_featmap': tgt_output,
            'supp_featmap': supp_output,
            'opflow_angle_mag': opflow_angle_mag,
        }
